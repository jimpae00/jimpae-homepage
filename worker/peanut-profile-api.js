...
async function adminSync(request, env) {
  ...
  const viewerStmt = env.DB.prepare(`
    ...
    (viewer_id, twitch_user_id, twitch_login, twitch_display_name, youtube_channel_id, youtube_handle, youtube_display_name, discord_user_id, discord_username, discord_linked, last_synced_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
  ...
  for (let i = 0; i < viewers.length; i += 100) {
    const batch = viewers.slice(i, i + 100).filter(v => v.viewer_id).map(v => viewerStmt.bind(
      ...
      v.discord_user_id || null, v.discord_username || null, v.discord_linked ? 1 : 0, syncedAt,
    ));
